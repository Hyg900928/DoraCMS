  ...
  async removes() {
    ...
    try {
      ...
      if (!_.isEmpty(oldUser)) {
        if (oldUser._id === ctx.session.adminUserInfo._id || leftAdminUser == 1) {
        ...
        if (this.app.config.env === 'local') {
          ...
          if (fs.existsSync(modulePkgPath)) {
            ...
            if (this.app.config.dev_modules.indexOf(pathItem) >= 0) {
              ...
              renderMap.push({
                name: pathItem,
                path: `${this.app.config.server_path +
                  this.app.config.static.prefix}/${pathItem}/js/app.js`
              });
            }
          } else {
            renderMap.push({
              ...
              path: `${this.app.config.origin +
                '/cms/plugins' +
                this.app.config.static.prefix}/admin/${pathItem}/js/app.js`
            });
          }
        } else {
          // 已安装的插件优先级最高
          ...
          if (plugins.indexOf(pluginStr) >= 0 && this.app.config[pluginStr].adminUrl) {
            let adminUrlItem =
              this.app.config.admin_root_path + this.app.config[pluginStr].adminUrl;
